...
def run_chunk_repair_tool():
    """Runs the Chunk Repair & Revision Tool.
    Args:
    None
    Returns:
    None
        ...
        while True:
            ...
            if choice == "1":
                ...
                def get_float_input(param_name, current_val, min_val=None, max_val=None):
                    """Prompt user for a float input with optional minimum and maximum values. Args: param_name (str): Name of the parameter current_val (float): Current value of the parameter min_val (float, optional): Minimum allowed value max_val (float, optional): Maximum allowed value Returns: float: Validated float value"""
                    while True:
                        try:
