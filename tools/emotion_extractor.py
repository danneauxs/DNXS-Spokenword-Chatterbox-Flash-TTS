    ...
    def select_best_segments(self, segments: List[EmotionalSegment], max_per_emotion: int = 10) -> Dict[str, List[EmotionalSegment]]:
        ...
        for emotion, emotion_segments in emotion_groups.items():
            ...
            def score_segment(seg):
                """Sorts emotional segments by a custom score and selects the best ones for each emotion category.
                Args:
                emotion_segments (Dict[str, List[EmotionalSegment]]): A dictionary where keys are emotions and values are lists of EmotionalSegment objects.
                output_dir (str): The directory to save combined samples.
                Returns:
                Dict[str, str]: A dictionary mapping emotions to paths of saved combined sample files.
                """
...
def main():
    """Extracts emotional voice samples from ChatterboxTTS output.
    Args:
    audiobook_dir (str): Path to audiobook directory containing TTS/ folder.
    -o --output (str, optional): Output directory for voice samples (default: extracted_samples).
    --min-duration (float, optional): Minimum segment duration (seconds) (default: 0.8).
    --max-per-emotion (int, optional): Maximum segments per emotion (default: 10).
    Returns:
    None
    """
